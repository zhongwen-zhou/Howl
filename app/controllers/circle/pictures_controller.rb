#encoding: utf-8
class Circle::PicturesController < Personal::ApplicationController
  ...
  def new
    @imageable = Group.find(params[:group_id].to_i)
  ...
  def create
    if params.has_key?(:activity_id)
      ...
      @group = Group.find(params[:group_id].to_i)
      @picture = @group.pictures.create(params[:picture])
      if @picture.valid?
        return redirect_to circle_groups_path(@group)
      else
